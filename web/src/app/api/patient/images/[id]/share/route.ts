...
export async function POST(request: Request, context: RouteContext<"/api/patient/images/[id]/share">) {
  ...
  if (!url || !key || !apiUrl) return NextResponse.json({ error: "Images are unavailable" }, { status: 503 });
  ...
  const response = await fetch(new URL("/api/share", apiUrl), {
    ...
    body: JSON.stringify({ resourceType: "image", resourceId: id, recipientEmail: body.recipientEmail }),
    cache: "no-store",
  });

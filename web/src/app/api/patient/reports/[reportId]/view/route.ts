...
export async function GET(request: Request, context: RouteContext<"/api/patient/reports/[reportId]/view">) {
  ...
  const response = await fetch(new URL(`/api/reports/${encodeURIComponent(reportId)}/view`, apiUrl), { headers: { authorization }, cache: "no-store" });

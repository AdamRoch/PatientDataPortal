...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!url || !key || !apiUrl) return NextResponse.json({ error: "Providers are unavailable" }, { status: 503 });
  ...
  const path = `/api/providers/${encodeURIComponent(id)}/slots?${query.toString()}`;
  const response = await fetch(new URL(path, apiUrl), { headers: { authorization }, cache: "no-store" });

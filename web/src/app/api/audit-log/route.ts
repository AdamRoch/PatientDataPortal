...
export async function GET(request: Request) {
  ...
  if (!url || !key || !apiUrl) return NextResponse.json({ error: "Audit log is unavailable" }, { status: 503 });
  ...
  const target = new URL("/api/audit-log", apiUrl);
  target.search = new URL(request.url).search;
  const response = await fetch(target, { headers: { authorization }, cache: "no-store" });

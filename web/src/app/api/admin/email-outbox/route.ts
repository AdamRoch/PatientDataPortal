...
export async function GET(request: Request) {
  ...
  if (!url || !key || !apiUrl) return NextResponse.json({ error: "Email outbox is unavailable" }, { status: 503 });

  const { data, error } = await createClient(url, key).auth.getUser(authorization.slice(7));
  if (error || !data.user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const response = await fetch(new URL("/api/admin/email-outbox", apiUrl), {

...
export async function proxyProviderAppointments(request: Request, path = "") {
  const authorization = request.headers.get("authorization");
  if (!authorization?.startsWith("Bearer ")) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const key = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  const apiUrl = process.env.API_URL;
  if (!url || !key || !apiUrl) return NextResponse.json({ error: "Appointments are unavailable" }, { status: 503 });
  ...
  const response = await fetch(new URL(path || "/api/provider/appointments", apiUrl), {
    method: request.method,
    headers: { authorization, ...(request.method === "GET" ? {} : { "content-type": "application/json" }) },

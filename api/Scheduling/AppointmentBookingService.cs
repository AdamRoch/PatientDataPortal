...
public sealed record CreateAppointmentRequest(Guid SlotId, Guid ServiceId, string IdempotencyKey, string? ReminderRecipientEmail = "patient@example.test");
...
public sealed class AppointmentBookingService(IOptions<DatabaseOptions> databaseOptions, IOptions<ReminderOptions> reminderOptions, IClock clock) : IAppointmentBookingService
{
    public async Task<AppointmentConfirmation> BookAsync(Guid patientUserId, CreateAppointmentRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (ReminderSchedule.IsDueBeforeStart(startsAt.Value, now, reminderOptions.Value))
                await ReminderSchedule.InsertAsync(connection, transaction, appointmentId, 1, startsAt.Value, request.ReminderRecipientEmail ?? throw new InvalidOperationException("An authenticated email address is required for reminders."), reminderOptions.Value, cancellationToken);
    ...
    private static Task InsertAppointmentAsync(NpgsqlConnection connection, NpgsqlTransaction transaction, Guid appointmentId, CreateAppointmentRequest request, Guid patientUserId, Guid providerId, DateTimeOffset startsAt, DateTimeOffset now, CancellationToken cancellationToken) =>
        ExecuteAsync(connection, transaction, "INSERT INTO appointments (id, slot_id, patient_user_id, provider_id, service_id, start_at, schedule_version, status, idempotency_key, reminder_recipient_email, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, 1, 'requested', $7, $8, $9, $9)", cancellationToken, appointmentId, request.SlotId, patientUserId, providerId, request.ServiceId, startsAt, request.IdempotencyKey, (object?)request.ReminderRecipientEmail ?? DBNull.Value, now);
